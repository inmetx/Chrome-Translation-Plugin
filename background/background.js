...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === "getMainFramePageLanguageState") {
    ...
    const currentTextTranslatorService = twpConfig.get("textTranslatorService");
    const nextTextServiceIndex = (enabledServices.indexOf(currentTextTranslatorService) + 1) % enabledServices.length;
    twpConfig.set("textTranslatorService", enabledServices[nextTextServiceIndex]);
    
    const currentPageTranslatorService = twpConfig.get("pageTranslatorService");
    const nextPageServiceIndex = (enabledServices.indexOf(currentPageTranslatorService) + 1) % enabledServices.length;
    twpConfig.set("pageTranslatorService", enabledServices[nextPageServiceIndex]);
...
//# sourceMappingURL=../maps/10.1.5.0/background.js.map
