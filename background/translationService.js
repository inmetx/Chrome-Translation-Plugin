...
const translationService = function () {
        ...
        doc.childNodes.forEach(node => {
          if (dontSortResults) {
            if (node.type === "text") {
              ...
              var _node$children;
              resultArray.push(currText + (((_node$children = node.children) === null || _node$children === void 0 || (_node$children = _node$children[0]) === null || _node$children === void 0 ? void 0 : _node$children.data) || ""));
              currText = "";
            }
          } else {
            if (node.type === "text") {
              ...
              resultArray[id] = currText + (((_node$children2 = node.children) === null || _node$children2 === void 0 || (_node$children2 = _node$children2[0]) === null || _node$children2 === void 0 ? void 0 : _node$children2.data) || "");
              currText = "";
            }
          }
        });
  ...
  const getSafeServiceByName = serviceName => {
    try {
      // 防御性检查，确保serviceName不为空
      if (!serviceName) return null;
      
      // 检查enabledServices是否为数组且包含此服务
      const enabledServices = twpConfig.get("enabledServices");
      const isServiceEnabled = Array.isArray(enabledServices) && enabledServices.includes(serviceName);
      
      // 检查customServices是否为数组且包含此服务
      const customServices = twpConfig.get("customServices");
      const isCustomService = Array.isArray(customServices) && 
                              customServices.some(cs => cs && typeof cs === 'object' && cs.name === serviceName);
      
      // 任一条件满足就返回服务
      if (isServiceEnabled || isCustomService) {
        return serviceList.get(serviceName);
      } else {
        return null;
      }
    } catch (error) {
      console.warn("Error in getSafeServiceByName:", error);
      return null;
    }
  };
  ...
  translationService.translateSingleText = async (serviceName, sourceLanguage, targetLanguage, originalText, dontSaveInPersistentCache = false) => {
    try {
      ...
      console.error("Error in translateSingleText:", error);
      return "";
    }
  };
  ...
  twpConfig.onReady(function () {
    ...
    try {
      const customServices = twpConfig.get("customServices");
      // 检查customServices是否是一个有效的数组，并且有find方法
      if (customServices && Array.isArray(customServices) && customServices.find) {
        if (customServices.find(cs => cs && cs.name === "libre")) {
          const libre = customServices.find(cs => cs && cs.name === "libre");
          if (libre && libre.url && libre.apiKey) {
            serviceList.set("libre", createLibreService(libre.url, libre.apiKey));
          }
        }
        
        if (customServices.find(cs => cs && cs.name === "deepl_freeapi")) {
          const deepl_freeapi = customServices.find(cs => cs && cs.name === "deepl_freeapi");
          if (deepl_freeapi && deepl_freeapi.apiKey) {
            serviceList.set("deepl", createDeeplFreeApiService(deepl_freeapi.apiKey));
          }
        }
      }
    } catch (error) {
      console.error("Error setting up custom translation services:", error);
    }
...
//# sourceMappingURL=../maps/10.1.5.0/translationService.js.map
