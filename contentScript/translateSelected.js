  ...
  try {
    if (typeof chrome !== 'undefined' && chrome && chrome.runtime && chrome.runtime.lastError) {
      ...
      if (chrome.runtime.lastError.message.includes("context invalidated") || 
        ...
        // 这些错误通常发生在页面重新加载或扩展上下文改变时，可以安全忽略
        console.debug("忽略运行时错误:", chrome.runtime.lastError.message);
      } else {
        // 记录其他错误供调试
        console.warn("运行时错误:", chrome.runtime.lastError.message);
      }
    }
  } catch (error) {
    // 即使在检查错误时也捕获可能的异常
    console.debug("checkedLastError函数内部错误:", error);
  }
}
  ...
  return new Promise((resolve) => {
    try {
      ...
      if (typeof chrome === 'undefined' || !chrome || !chrome.runtime) {
        console.warn("Chrome API不可用，无法翻译文本");
        resolve(null);
        return;
      }
      ...
      chrome.runtime.sendMessage(
        ...
        response => {
          ...
          if (chrome.runtime.lastError) {
            console.debug("翻译请求发生错误（非关键）: " + chrome.runtime.lastError.message);
            resolve(null);
            return;
          }
...
Promise.all([twpConfig.onReady(), getTabHostName()]).then(function (_) {
  ...
  window.addEventListener("focus", function (e) {
    ...
    // 使用Promise实现超时处理
    function sendMessageWithTimeout(message, timeout = 1000) {
      return new Promise((resolve) => {
        // 防御性检查chrome API是否可用
        if (typeof chrome === 'undefined' || !chrome || !chrome.runtime) {
          console.debug("Chrome API不可用，无法发送消息");
          resolve(null);
          return;
        }
        ...
        // 设置超时处理
        const timeoutId = setTimeout(() => {
          console.debug("消息发送超时：" + message.action);
          resolve(null);
        }, timeout);
          ...
          chrome.runtime.sendMessage(message, (response) => {
            ...
            if (chrome.runtime.lastError) {
              ...
              if (errorMsg.includes("context invalidated") || 
                ...
                console.debug("发送消息时出现非关键错误:", errorMsg);
              } else {
                console.debug("发送消息时出现错误:", errorMsg);
              }
              resolve(null);
            } else {
              resolve(response);
    ...
    sendMessageWithTimeout({
      ...
      console.debug("通知框架焦点状态时出错:", error);
    });
  });
...
window.addEventListener('unhandledrejection', function(event) {
  ...
  try {
    ...
    console.debug('捕获到未处理的Promise拒绝:', errorInfo);
  } catch (e) {
